...
SELECT COUNT(user_id), COUNT(user_first_touch_timestamp) FROM users_dirty
GROUP BY user_id, user_first_touch_timestamp;

-- COMMAND ----------

-- DBTITLE 1,To check unique user_id with unique user_first_touch_timestamp to get the non duplicate values
...
SELECT email, count(email_id) AS 
FROM deduped_users
...
SELECT *, CAST(user_first_touch_timestamp/1e6 AS timestamp) AS user_first_touch FROM users_dirty;
